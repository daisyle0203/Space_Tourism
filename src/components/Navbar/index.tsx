...
export default function Navbar() {
  ...
  // Custom hook to check if the current path is active or not
  const isActive = useCallback((href: string) => path === href, [path])

  return (
    <header
      className={`relative z-30 flex justify-between items-center px-6 md:px-0 md:pl-10 lg:pt-10 group ${
        opened && "menu-open"
      }`}
    >
      <Image
      ...
      <button onClick={toggle} className="relative z-20 w-6 h-6 md:hidden">
        <Image
          src={hamburger}
          alt="Icon Menu Open"
          className="transition group-[.menu-open]:opacity-0 absolute top-1/2 right-0 -translate-y-1/2 w-full h-full"
        />
        <Image
          src={close}
          alt="Icon Menu Close"
          className="group-[:not(.menu-open)]:opacity-0 transition absolute top-1/2 right-0 -translate-y-1/2 w-full h-full"
        />
      </button>
      <div className="bar grow hidden lg:block h-0.5 bg-white/60 translate-x-8 relative z-10"></div>
      <ul
        className="list-none transition backdrop-blur-xl translate-x-full fixed top-0 right-0 h-screen w-64 z-0 pt-28 pl-8 group-[.menu-open]:translate-x-0
      ...
      >
        {pages.map(({ title, href }, index) => (
          ...
          >
            ...
            >
              <span className="block font-bold min-w-[20px] md:hidden">
                0{index}
              </span>
