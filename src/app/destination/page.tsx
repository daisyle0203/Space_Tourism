...
import PageTitle from "@/components/NumberedTitles"
import { useMemo, useState } from "react"
import MotionDiv from "@/components/MotionDiv"
import { AnimatePresence, motion } from "framer-motion"
import { SLIDE_LEFT, SLIDE_RIGHT } from "@/utils"
...
export default function Destination() {
  ...
  const previous = usePrevious(start)
  const planet = useMemo(() => planets[start], [start])
  const variant = useMemo(() => {
    if (previous === undefined) return SLIDE_RIGHT
    if (start > previous) return SLIDE_LEFT
    return SLIDE_RIGHT
  }, [start])
      ...
      <main className="relative z-10 p-6 md:px-24 md:pb-0">
        ...
        <div className="grid gap-40 lg:grid-cols-2 lg:pt-12">
          <AnimatePresence>
            ...
            >
              <Image
                className="w-40 md:w-80 lg:w-[440px] aspect-square"
                src={planet.image}
                alt="Europa"
              />
            </motion.div>
